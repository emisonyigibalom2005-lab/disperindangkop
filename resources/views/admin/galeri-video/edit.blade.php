...
<div class="container-fluid">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between text-white">
                        ...
                        <div class="text-right d-none d-md-block">
                            <a href="{{ route('admin.galeri-video.index') }}" class="btn btn-light btn-modern">
    ...
    <form action="{{ route('admin.galeri-video.update', $galeriVideo->id) }}" method="POST" id="formVideo">
        @csrf
        @method('PUT')
            ...
            <div class="col-lg-9">
                ...
                <div class="content-card mb-4">
                    ...
                    <div class="content-card-body">
                        <div class="video-preview-container">
                            @php
                                $videoId = '';
                                if (preg_match('/youtube\.com\/watch\?v=([^&]+)/', $galeriVideo->video_url, $matches)) {
                                    $videoId = $matches[1];
                                } elseif (preg_match('/youtu\.be\/([^?]+)/', $galeriVideo->video_url, $matches)) {
                                    $videoId = $matches[1];
                                }
                            @endphp
                            @if($videoId)
                        ...
                        <div class="text-center mt-3">
                            <small class="text-muted">
                                ...
                                URL: <a href="{{ $galeriVideo->video_url }}" target="_blank">{{ $galeriVideo->video_url }}</a>
                            </small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="content-card-body">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('judul', $galeriVideo->judul) }}"
                                   required>
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="url" 
                                   ...
                                   value="{{ old('video_url', $galeriVideo->video_url) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="kategori" class="form-control form-control-modern">
                                        <option value="kegiatan" {{ $galeriVideo->kategori == 'kegiatan' ? 'selected' : '' }}>Kegiatan</option>
                                        <option value="pelatihan" {{ $galeriVideo->kategori == 'pelatihan' ? 'selected' : '' }}>Pelatihan</option>
                                        <option value="tutorial" {{ $galeriVideo->kategori == 'tutorial' ? 'selected' : '' }}>Tutorial</option>
                                        <option value="dokumentasi" {{ $galeriVideo->kategori == 'dokumentasi' ? 'selected' : '' }}>Dokumentasi</option>
                                        <option value="lainnya" {{ $galeriVideo->kategori == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <textarea name="deskripsi" class="form-control form-control-modern" rows="3" placeholder="Tambahkan deskripsi singkat...">{{ old('deskripsi', $galeriVideo->deskripsi) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="content-card mt-4">
                    <div class="content-card-body" style="background:#f8f9ff;border-top:1px solid #eef;display:flex;justify-content:space-between;align-items:center;padding:25px 30px">
                        <a href="{{ route('admin.galeri-video.index') }}" class="btn btn-secondary btn-modern">
