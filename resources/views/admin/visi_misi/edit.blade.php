...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0"><i class="fas fa-edit mr-2"></i>Edit Visi & Misi</h1>
            </div>
            <div class="col-sm-6">
...
<div class="content">
    <div class="container-fluid">
        <form method="POST" action="{{ route('admin.visi-misi.update', $visiMisi->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <textarea name="visi" id="visi" class="form-control @error('visi') is-invalid @enderror" rows="6"
                                          placeholder="Tuliskan visi organisasi..." required>{{ old('visi', $visiMisi->visi) }}</textarea>
                                @error('visi')
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="misi" id="misi" class="form-control @error('misi') is-invalid @enderror" rows="10"
                                          placeholder="Tuliskan misi organisasi (pisahkan dengan enter untuk setiap poin)..." required>{{ old('misi', $visiMisi->misi) }}</textarea>
                                @error('misi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="form-text text-muted">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-control @error('status') is-invalid @enderror" required>
                                    <option value="aktif" {{ old('status', $visiMisi->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                    <option value="nonaktif" {{ old('status', $visiMisi->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="alert alert-info">
                                ...
                                <ul class="mb-0 pl-3">
                                    ...
                                    <li>Terakhir diupdate: <br><strong>{{ $visiMisi->updated_at->format('d M Y, H:i') }} WIT</strong></li>
                                </ul>
                            </div>

                            <div class="d-grid gap-2">
                                <button type="submit" class="btn btn-warning btn-block text-white">
                                    <i class="fas fa-save mr-1"></i> Update Visi & Misi
                                </button>
